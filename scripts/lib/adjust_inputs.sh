...
if [ -n "$KERNEL_NAME" ]; then
  ...
  grep -qF "CONFIG_LOCALVERSION=\"$KERNEL_NAME\"" arch/arm64/configs/ghost_defconfig \
    || { echo "[-] sed gagal set CONFIG_LOCALVERSION — pattern gak ketemu di ghost_defconfig, cek struktur defconfig"; exit 1; }
fi

if [ "$SPOOF_UNAME" == "on" ]; then
  sed -i "s/# CONFIG_KSU_SUSFS_SPOOF_UNAME is not set/CONFIG_KSU_SUSFS_SPOOF_UNAME=y/g" \
    arch/arm64/configs/ghost_defconfig
  grep -qF "CONFIG_KSU_SUSFS_SPOOF_UNAME=y" arch/arm64/configs/ghost_defconfig \
  ...
  sed -i "s/CONFIG_KSU_SUSFS_SPOOF_UNAME=y/# CONFIG_KSU_SUSFS_SPOOF_UNAME is not set/g" \
    arch/arm64/configs/ghost_defconfig
  grep -qF "# CONFIG_KSU_SUSFS_SPOOF_UNAME is not set" arch/arm64/configs/ghost_defconfig \
